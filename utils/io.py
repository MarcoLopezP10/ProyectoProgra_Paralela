...
def save_swarm_trajectory_npz(
    *,
    iteration_numbers: List[int],
    positions: List[np.ndarray],
    global_bests: List[np.ndarray],
    fitness_history: List[float],
    iteration_records: List[Dict[str, Any]],
    out_path: str,
) -> None:
    """Persist sampled swarm trajectories in a compact NumPy archive."""
    _ensure_parent_dir(out_path)
    ...
    global_bests_array = (
        np.asarray(global_bests, dtype=float)
        if global_bests
        else np.empty((0, 0), dtype=float)
    )

    def _metric_series(key: str) -> np.ndarray:
        return np.asarray(
            [float(row.get(key, 0.0)) for row in iteration_records],
            dtype=float,
        )

    np.savez_compressed(
        out_path,
        iteration_numbers=np.asarray(iteration_numbers, dtype=int),
        positions=positions_array,
        global_bests=global_bests_array,
        fitness_history=np.asarray(fitness_history, dtype=float),
        eval_s=_metric_series("eval_s"),
        update_s=_metric_series("update_s"),
        overhead_s=_metric_series("overhead_s"),
        iter_s=_metric_series("iter_s"),
        stall_count=_metric_series("stall_count"),
    )


def load_swarm_trajectory_npz(out_path: str) -> Dict[str, np.ndarray]:
    """Load a trajectory archive generated by save_swarm_trajectory_npz()."""
